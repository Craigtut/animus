...
| `docs/research/sub-agent-delegation-and-tick-efficiency.md` | Plan to stop the mind re-deriving context every tick (94% of file reads were cross-tick re-reads when doing long-running work inline). Consolidate to Cortex's in-loop `SubAgent` tool extended via 3 general-purpose Cortex hooks (pre-spawn augmentation, opaque metadata round-trip, `getActiveSubAgents()` introspection); fully remove the legacy reflect-phase `spawn_agent` decision; seed sub-agents with background context (not the goal as a directive); sub-agent spend (incl. in-flight) counts toward the weekly budget with no per-sub-agent caps; move goals/tasks into ephemeral; thread a stable `sessionId` so `prompt_cache_key` engages on gpt-5.5. **STATUS: Proposal, not implemented.** |
