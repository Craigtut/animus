...
export default function createAdapter(ctx: AdapterContext): ChannelAdapter {
  ...
  const enableOllama = (ctx.config['enableOllama'] as boolean | undefined) ?? true;
  const enableOpenai = (ctx.config['enableOpenai'] as boolean | undefined) ?? true;
  ...
  function waitForReply(requestId: string): Promise<string> {
  ...
  function validateApiKey(request: RouteRequest): boolean {
    ...
    async start(): Promise<void> {
      ...
      if (enableOpenai) {
        ...
        ctx.registerRoute({
          ...
          handler: async (_request: RouteRequest): Promise<RouteResponse> => {
            return {
              status: 200,
              body: {
                object: 'list',
        ...
        ctx.registerRoute({
          ...
          ): Promise<RouteResponse | StreamingRouteResponse> => {
      ...
      ctx.log.info(
        `API adapter started (OpenAI: ${enableOpenai}, Ollama: ${enableOllama})`,
      );
    },
