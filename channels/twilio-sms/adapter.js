...
export default function createAdapter(ctx) {
    const accountSid = ctx.config['accountSid'];
    const authToken = ctx.config['authToken'];
    const phoneNumber = ctx.config['phoneNumber'];
    return {
        async start() {
            ...
            ctx.registerRoute({
                ...
                handler: async (request) => {
                    ...
                    if (signature) {
                        const webhookUrl = ctx.config['webhookUrl'];
                        if (webhookUrl) {
        ...
        async send(contactId, content, _metadata) {
