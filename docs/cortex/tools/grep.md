...
|-----------|------|:--------:|-------------|
| `pattern` | string | Yes | Regex pattern to search for (ripgrep syntax) |
...
| `outputMode` | string | No | `files_with_matches` (default), `content`, or `count` |
| `contextLines` | number | No | Lines of context before/after each match. Only in `content` mode. |
| `caseSensitive` | boolean | No | Default: true. Set false for case-insensitive. |
| `maxResults` | number | No | Limit number of results. Default: unlimited. |
| `offset` | number | No | Skip first N results before applying maxResults. Enables pagination. |
...
- The `offset` + `maxResults` pattern enables pagination through large result sets without loading everything into context.
