...
packages/backend/src/speech/
  ...
  tts-engine.ts      -- TTSEngine class (lazy-loaded @animus/tts-native PocketTTS, voice state caching)
...
packages/tts-native/            -- napi-rs native addon wrapping pocket-tts Rust crate
  ...
  package.json                  -- @animus/tts-native npm package
...
Lazy-loads `@animus/tts-native`'s `PocketTTS` model on first `synthesize()` call via dynamic `import('@animus/tts-native')`. The native addon is compiled per-platform (requires Rust toolchain for first build). If the addon is not built, `ensureLoaded()` throws with a clear error message.
...
2. **No crash on missing native modules** -- STT uses dynamic `import('sherpa-onnx-node')` and TTS uses dynamic `import('@animus/tts-native')`. If either native addon is not built/installed, the dynamic import fails gracefully and that engine reports as unavailable.
...
| @animus/tts-native not built | TTS engine reports unavailable with message: "Native TTS addon not built. Run: npm run build -w @animus/tts-native (requires Rust toolchain)". Backend runs normally. |
...
We investigated whether ONNX-based Pocket TTS could replace our custom Rust/napi-rs binding (`@animus/tts-native`). Three approaches were tested:
