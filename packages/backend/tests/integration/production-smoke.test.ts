  ...
  beforeAll(async () => {
    ...
    proc = spawn('node', [BACKEND_DIST], {
      env: {
        ...
        PORT: String(port),
        HOST: '127.0.0.1',
        DB_SYSTEM_PATH: join(tmpDir, 'system.db'),
        DB_HEARTBEAT_PATH: join(tmpDir, 'heartbeat.db'),
        DB_MEMORY_PATH: join(tmpDir, 'memory.db'),
        DB_MESSAGES_PATH: join(tmpDir, 'messages.db'),
        DB_AGENT_LOGS_PATH: join(tmpDir, 'agent_logs.db'),
        DB_PERSONA_PATH: join(tmpDir, 'persona.db'),
        LANCEDB_PATH: join(tmpDir, 'lancedb'),
